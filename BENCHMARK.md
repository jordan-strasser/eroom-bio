...
- **Nested CV:** tune hyperparameters (N_eff tiers, field bandwidth, softmin temperature, safety weights) on **validation folds only**; report on a **test fold never seen during tuning**. NEVER tune on the headline holdout — that's the round-25/28 anti-pattern (`[[project_round28_sicko_mode]]`, `[[project_round25_evidence_records]]`).
